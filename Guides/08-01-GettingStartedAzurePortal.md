...
|---|---|---|
| Resource Group | `<prefix>-rg-<env>` | `on-rg-dev` |
| Policy assignment | `<prefix>-policy-<description>` | `on-policy-tags` |
| Role assignment description | Use your prefix when documenting | — |
| Budget alert | `<prefix>-budget-lab` | `on-budget-lab` |

> **Why this matters:** Tags alone are not enough if names collide. Consistent naming lets the instructor audit the environment and lets Cost Management filter by student.
...
2. Sign in with the student account provided by your instructor.
3. If you received an invitation email to the NordStack AS tenant, accept it before continuing. You may need to switch directory — click your account icon in the top right and select **Switch directory**.
...
> **Reflection:** In the on-premises lab you logged in with your individual `adm_<username>` account rather than a shared Administrator account. The same principle applies here — never share your Azure credentials, and always know which identity performed which action.
...
You will create three Resource Groups representing different environments within NordStack AS.
...
| Name | Region | Purpose |
|---|---|---|
| `<prefix>-rg-dev` | Norway East | Development workloads |
| `<prefix>-rg-test` | Norway East | Testing and validation |
| `<prefix>-rg-prod` | Norway East | Production (restricted) |

3. For each Resource Group, proceed to the **Tags** tab during creation and add the following tags:

| Tag name | Value |
|---|---|
| `Environment` | `dev` / `test` / `prod` (match the group) |
| `CostCenter` | `lab` |
| `Owner` | Your student username |
| `Project` | `nordstack-lab` |

4. Complete the creation and verify all three groups appear in the Resource Groups list.

> **Why three environments?** This mirrors a real enterprise pattern. Policies and RBAC assignments often differ between dev (permissive), test (moderate), and prod (strict). You will apply increasingly restrictive policies as you progress through this lab.
...
1. Open your `<prefix>-rg-dev` Resource Group.
2. In the left menu, select **Access control (IAM)**.
3. Click **View my access**. Note which role you have been assigned and at which scope.

### 3.2 Test RBAC by assigning a role to a classmate

Work with one of your classmates for this step.

1. In `<prefix>-rg-dev`, go to **Access control (IAM)** → **+ Add** → **Add role assignment**.
2. Select the **Reader** role. Click **Next**.
3. Under **Members**, click **+ Select members** and find your classmate's account.
4. Complete the assignment.

5. Ask your classmate to navigate to *your* Resource Group in their portal session. Verify they can see the contents but cannot create or delete anything.

6. Return to IAM and **remove** the Reader assignment when done.

> **Reflection:** This is equivalent to removing a user from a Domain Local group on a file server. Access is revoked immediately — no reboot or logoff required. Compare this to GPO processing, which requires a policy refresh cycle.

### 3.3 Understand role scope inheritance
...
## Phase 4 – Azure Policy Guardrails
...
### 4.1 Allowed Locations — scoped to `rg-prod`
...
4. Under **Parameters → Allowed locations**, select only **Norway East**.
5. Name the assignment: `<prefix>-policy-location-prod`
6. Leave enforcement mode as **Enabled**. Assign.

**Test it:**
1. Inside `<prefix>-rg-prod`, try to create a simple resource (e.g., a Storage Account) and choose **UK South** as the region.
...
### 4.2 Tag Enforcement — scoped to `rg-dev`

In Phase 2 you manually added tags to your Resource Groups. Now you will enforce that any *resource* created inside `rg-dev` must also carry the `Owner` tag.

1. Go to **Policy** → **Assignments** → **+ Assign policy**.
2. Set **Scope** to your `<prefix>-rg-dev` Resource Group.
3. Search for the built-in definition: **Require a tag on resources**.
4. Under **Parameters → Tag Name**, enter: `Owner`
5. Name the assignment: `<prefix>-policy-tag-owner-dev`
6. Assign.
