...
## Del 4: Test Scenarios

### Scenario 1: Force Update Installation

**Oppgave:** Tving cl1 til å installere alle pending updates umiddelbart.
...
Invoke-Command -ComputerName cl1.infrait.sec -ScriptBlock {
    ...
    # Tvinge update check og download
    UsoClient StartScan
    UsoClient StartDownload
    
    # Installer alle pending updates
    UsoClient StartInstall
    
    Write-Host "Update installasjon startet. Sjekk status om 5 minutter." -ForegroundColor Green
}
```

**Verifiser etter 5 minutter:**
```powershell
Invoke-Command -ComputerName cl1.infrait.sec -ScriptBlock {
    Get-WindowsUpdateLog
    # Eller sjekk Event Log
    Get-WinEvent -LogName System -FilterHashtable @{ProviderName='Microsoft-Windows-WindowsUpdateClient'; ID=19} -MaxEvents 5
}
```

---

### Scenario 2: Pause Updates (Maintenance Window)

**Oppgave:** Pause updates på srv1 i 7 dager mens du gjør vedlikehold.

1. I GPMC, edit `Corporate - Windows Update Policy`
2. Gå til "Select when Quality Updates are received"
3. Sett **"Pause Quality Updates starting"** til dagens dato
4. Kjør `gpupdate /force` på srv1
5. Verifiser at updates er pauset:

```powershell
Invoke-Command -ComputerName srv1.infrait.sec -ScriptBlock {
    $Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate'
    Get-ItemProperty -Path $Path | Select PauseQualityUpdatesStartTime
}
```

**Husk å fjerne pause etter vedlikehold!**
